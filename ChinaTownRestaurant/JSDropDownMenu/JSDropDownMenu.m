...
#define SelectColor [UIColor clearColor]
...
@property (nonatomic) BOOL isLeftest;//判断是否为菜品，如果是，则有图片
@property (nonatomic) int countForFood;//为了判断是否为菜品，计数用的
...
//        CATextLayer *title = [self createTextLayerWithNSString:titleString withColor:self.textColor andPosition:titlePosition];
        CATextLayer *title = [self createTextLayerWithNSString:titleString withColor:[UIColor whiteColor] andPosition:titlePosition];//自定义，menu的标题字体的颜色
...
//        _leftTableView.backgroundColor = [UIColor colorWithRed:106.0/256.0 green:100.0/256.0 blue:105.0/256.0 alpha:1.0];
...
//    int x = 0; int y = 0;  int toX = 4;  int toY = 5;
...
//    lineShape1.lineWidth = 1.0f;
//    lineShape1.lineCap = kCALineCapRound;
//    lineShape1.strokeColor = [UIColor whiteColor].CGColor;
//    int x1 = 4; int y1 = 5;  int toX1 = 8;  int toY1 = 0;
//    CGPathMoveToPoint(linePath1, NULL, x1, y1);
//    CGPathAddLineToPoint(linePath1, NULL, toX1, toY1);
//    lineShape1.path = linePath1;
//    CGPathRelease(linePath1);
//    [layer addSublayer:lineShape1];
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (leftTableView) {
            ...
            //根据数组个数来制定高度
            int count = (ScreenHeight -64-40)/leftTableView.rowHeight;
            leftTableViewHeight = ([leftTableView numberOfRowsInSection:0] > count) ? ((count-4) * leftTableView.rowHeight) : ([leftTableView numberOfRowsInSection:0] * leftTableView.rowHeight);
//            int count = (ScreenHeight -64-40)/leftTableView.rowHeight-2;
//            if ([leftTableView numberOfRowsInSection:0] >= count) {
//                leftTableViewHeight = (count-3)*leftTableView.rowHeight;
//            }else{
//                leftTableViewHeight = leftTableView.rowHeight*count;
//            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //自定义，cell背景图片
    UIImageView *bgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, self.frame.size.height)];
    bgView.image = [[UIImage imageNamed:@"recommendChooseCellBg"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    [cell.contentView addSubview:bgView];
    
    UILabel *titleLabel = [[UILabel alloc] init];
//    titleLabel.textColor = self.textColor;//自定义cell上的Label颜色
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        ...
        //自定义菜品下拉菜单icon
        NSString *maxStr = @"小吃面点";
        CGSize iconX = [maxStr textSizeWithFont:[UIFont systemFontOfSize:14.0] constrainedToSize:CGSizeMake(MAXFLOAT, 14) lineBreakMode:NSLineBreakByWordWrapping];
        CGFloat ratio = [_dataSource widthRatioOfLeftColumn:_currentSelectedMenudIndex];
        CGFloat marginX = (self.frame.size.width*ratio-iconX.width)/2;
        CGFloat margin = 10;
        ...
        if ([titleLabel.text isEqualToString:@"全部"]) {
            ...
            icon.image = [UIImage imageNamed:@"menu_xiaochi"];
            [cell addSubview:icon];
        }else if ([titleLabel.text isEqualToString:@"粤菜"]) {
            icon.image = [UIImage imageNamed:@"menu_yue"];
            [cell addSubview:icon];
        }else if ([titleLabel.text isEqualToString:@"川菜"]) {
            icon.image = [UIImage imageNamed:@"menu_sichuan"];
            [cell addSubview:icon];
        }else if ([titleLabel.text isEqualToString:@"东北菜"]) {
            icon.image = [UIImage imageNamed:@"menu_dongbei"];
            [cell addSubview:icon];
        }else if ([titleLabel.text isEqualToString:@"江浙菜"]) {
            icon.image = [UIImage imageNamed:@"menu_jiangzhe"];
            [cell addSubview:icon];
        }
    ...
    if (leftOrRight == 1) {
        ...
        if ([titleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
            ...
            titleLabel.textColor = [UIColor yellowColor];//自定义选中cell颜色
