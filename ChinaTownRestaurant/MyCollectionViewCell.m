...
- (instancetype) initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.blackLineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 0)];
...
//        _adressAndPriceView.backgroundColor = UIColorFromRGB(0x000000, 0.20);
        _adressAndPriceView.backgroundColor = [UIColor blackColor];
...
- (void) setModel:(MyCollectionModel *)model{
    ...
    if ([model.category isEqualToString:@"小吃面食"]) {
    ...
    }else if ([model.category isEqualToString:@"西北菜"]){
        _iconImageView.image = [UIImage imageNamed:@"cell_icon_dongbei"];
    }else if ([model.category isEqualToString:@"江浙菜"]){
    ...
    CGFloat rating = model.avgRating.floatValue/10;
