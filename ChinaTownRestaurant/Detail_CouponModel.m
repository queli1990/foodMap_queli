...
@implementation Detail_CouponModel
...
+ (Detail_CouponModel *)modelWithDictionary:(NSDictionary *)dictionary{
    Detail_CouponModel *model = [[Detail_CouponModel alloc]initWithDictionary:dictionary];
    return model;
}
...
+ (NSArray *)modelsWithArray:(NSArray *)usersArray{
    ...
    if (usersArray.count >= 1) {
        for (int i = 0; i < usersArray.count; i++) {
            Detail_CouponModel *model = [Detail_CouponModel modelWithDictionary:usersArray[i]];
